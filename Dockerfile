...
CMD ["sh", "-c", "CFGDIR=${OPENCLAW_STATE_DIR:-/home/node/.openclaw} && mkdir -p $CFGDIR && node -e \"const fs=require('fs');const d=process.env.OPENCLAW_STATE_DIR||'/home/node/.openclaw';const p=d+'/openclaw.json';let c={};try{c=JSON.parse(fs.readFileSync(p))}catch(e){}c.gateway=c.gateway||{};c.gateway.controlUi={dangerouslyAllowHostHeaderOriginFallback:true};c.gateway.auth=c.gateway.auth||{};c.gateway.auth.mode='token';fs.writeFileSync(p,JSON.stringify(c));console.log('[ema] config patched at '+p)\" && node openclaw.mjs gateway --allow-unconfigured --bind lan --port ${PORT:-10000}"]
