...
CMD ["sh", "-c", "mkdir -p /home/node/.openclaw && node -e \"const fs=require('fs');const p='/home/node/.openclaw/openclaw.json';let c={};try{c=JSON.parse(fs.readFileSync(p))}catch(e){}c.gateway=c.gateway||{};c.gateway.controlUi={dangerouslyAllowHostHeaderOriginFallback:true};c.gateway.auth=c.gateway.auth||{};c.gateway.auth.mode='token';fs.writeFileSync(p,JSON.stringify(c));console.log('[ema] config patched')\" && node openclaw.mjs gateway --allow-unconfigured --bind lan --port ${PORT:-10000}"]
